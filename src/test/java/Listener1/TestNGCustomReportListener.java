...
import org.testng.xml.XmlSuite;
	 ...
	 * "emailable-FON-report" + DateFunctions.dateToDayAndTimeForFileName(now) +
	 * ".html")))); }
	 */
		...
		private void resultDetail(IResultMap tests)
		{
			...
			for (ITestResult result : testResultsList) 
			{
				...
				ITestNGMethod method = result1.getMethod();
				m_methodIndex++;
				String cname = method.getTestClass().getName();
				writer.println("<h2 id=" + "m" + m_methodIndex + ">" + cname + ":"
						+ method.getMethodName() + "</h2>");
				Set<ITestResult> resultSet = tests.getResults(method);
				generateResult(result1, method, resultSet.size());
		...
		private void getParameters(IResultMap tests) {
	        ...
	        for (ITestResult result : tests.getAllResults()) {
	            ...
	            if (hasParameters) {
	                ...
	                for (Object p : parameters) {
	                    writer.println(Utils.escapeHtml(Utils.toString(p)) + " | ");
	                }
	            }
	    ...
	    public void generateSuiteSummaryReport(List<ISuite> suites) {
	        ...
	        for (ISuite suite : suites) 
	        {
	            ...
	            for (ISuiteResult r : tests.values()) 
	            {
	                ...
	                // NEW
				/*
				 * // Insert error found writer.print("<td class=\"numi" + (true ? "" : "_attn")
				 ...
				 */
	                ...
	                writer.print("<td class=\"numi" + (true ? "" : "_attn") + "\">");
